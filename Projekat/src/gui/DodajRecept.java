...
public class DodajRecept extends JDialog {
	...
	private JComboBox<Object> lekar;
	...
	public static void main(String[] args) {
		try {
			DodajRecept dialog = new DodajRecept();
	...
	public DodajRecept() {
		...
		contentPanel.setLayout(gbl_contentPanel);
		{
			...
			lblNewLabel.setIcon(new ImageIcon(DodajRecept.class.getResource("/slike/imelaLogo.jpg")));
		...
		{
			JLabel lblNewLabel_1 = new JLabel("Dodavanje recepta");
		...
		{
			ArrayList<String> spisakLekara = new ArrayList<String>();
			for (Korisnik k : ListaKorisnika.getInstance().getKorisnici()) {
				if (k.getTip().equals("Lekar")) {
				spisakLekara.add(k.getUsername());
				}
			}
			String[] lista = spisakLekara.toArray(new String[spisakLekara.size()]);
			System.out.println(lista);
			
			lekar = new JComboBox<Object>(lista);
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for (Recept l : ListaRecepata.getInstance().getRecepti()) {

							if (sifra.getText().equals(l.getSifra())) {
								JOptionPane.showMessageDialog(null, "Vec postoji recept sa sifrom " + sifra.getText(),
						...
						try {
							sifra.setText(String.valueOf(Recept.getMaxSifra() + 1));
						} catch (ParseException e2) {
							// TODO Auto-generated catch block
							e2.printStackTrace();
						}
						try {
							ReceptiController.getInstance().dodajRecept(sifra.getText(), (String) lekar.getSelectedItem(),
