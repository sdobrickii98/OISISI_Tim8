...
public class IzmeniKorisnika extends JDialog {
	...
	public IzmeniKorisnika(Korisnik korisnik) {
		...
		contentPanel.setLayout(gbl_contentPanel);
		{
			...
			lblNewLabel.setIcon(new ImageIcon(IzmeniKorisnika.class.getResource("/slike/imelaLogo.jpg")));
		...
		{
			JLabel lblNewLabel_1 = new JLabel("Izmena korisnika");
		...
		}
		{
			...
			tip.setSelectedItem(korisnik.getTip());
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (!KorisniciJTable.getInstance().getSelectionModel().isSelectionEmpty()) {
							...
							KorisniciController.getInstance().izmeniKorisnika(username.getText(), password.getText(),
									ime.getText(), prezime.getText(), izabranTip, k);
							dispose();
						} else {
