...
public class DodajLekNaRecept extends JDialog {
	...
	private void jbInit() throws Exception {
		...
		title = new JLabel("Recept - DodajLek");
		...
		b2.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				LekoviController.getInstance().DodavanjeLekaNaRecept(recept, (String) tf1.getSelectedItem());
				AbstractTableModelRecepti model = (AbstractTableModelRecepti) ReceptiJTable.getInstance().getModel();
				Float ukupno = (float) 0;
				for (String lista : recept.getSpisakLekova()) {	
					for (Lek l : ListaLekova.getInstance().getLekovi()) {
				...
				recept.setUkCena(ukupno);
				}	
				model.setValueAt(ukupno, row, 4);
				dispose();
			}
