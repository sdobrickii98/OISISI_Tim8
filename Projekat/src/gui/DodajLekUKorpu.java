...
public class DodajLekUKorpu extends JDialog {
	...
	private void jbInit() throws Exception {
		...
		title = new JLabel("Korpa - DodajLek");
		...
		b2.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Prodavnica prodavnica = ListaProdavnica.getInstance().getRow(rowprod);
					LekoviController.getInstance().DodavanjeLekaUKorpu(prodavnica, (String) tf1.getSelectedItem());
					...
					for (String lista : prodavnica.getSpisakLekova()) {	
						for (Lek l : ListaLekova.getInstance().getLekovi()) {
					...
					prodavnica.setUkCena(ukupno);
					}	
					model.setValueAt(ukupno, rowprod, 4);					
				dispose();
			}
